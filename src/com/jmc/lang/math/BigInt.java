...
public class BigInt {
    ...
    public BigInt(String a) {
        ...
        this.data = new int[isPositive ? a.length() : a.length() - 1];
        if (isPositive)
            for (int i = 0; i < data.length; i++) data[i] = cs[i] - '0';
        else
            for (int i = 0; i < data.length; i++) data[i] = cs[i + 1] - '0';
    }

    public static BigInt valueOf(long a) {
        if (a == 0) return new BigInt(new int[] {0}, true);
        boolean isPositive = a > 0;
        int[] result = new int[19];
        int p = result.length - 1;

        while (a != 0) {
            result[p--] = isPositive ? (int) (a % 10) : (int) (-a % 10);
            a /= 10;
        }

        if (p != -1) result = Arrays.copyOfRange(result, p + 1, result.length);
    ...
    private BigInt sub(int[] data1, int[] data2) {
        if (isZero(data1)) return new BigInt(data2, false);
        if (isZero(data2)) return new BigInt(data1, true);
        if (data1.length < data2.length) return BigInt.valueOf(0).sub(sub(data2, data1));
        ...
        if (carry == -1) return BigInt.valueOf(0).sub(sub(data2, data1));
        ...
        if (result[0] == 0) {
            ...
            if (countZero == result.length) return BigInt.valueOf(0);
    ...
    private int[] div(int[] data1, int[] data2) {
        if (isZero(data2)) throw new ArithmeticException("BigInt divide by zero!");
        if (data1.length < data2.length) return new int[] {0};
